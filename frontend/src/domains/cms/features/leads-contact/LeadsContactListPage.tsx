...
export default function LeadsContactListPage() {
  const [tab, setTab] = useState<CmsListTab>("active");
  const [rows, setRows] = useState<LeadsContact[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res =
        tab === "deleted"
          ? await cmsApi.listDeletedLeadsContact({
              page,
              limit: PAGE_SIZE,
              search: search || undefined,
            })
          : await cmsApi.listLeadsContact({
              page,
              limit: PAGE_SIZE,
              search: search || undefined,
            });
      ...
      const message = cmsErrorMessage(err, "Failed to load contact leads");
      setError(message);
  ...
  const onDelete = async (id: string, name: string) => {
    ...
    if (!ok) return;
    try {
      const res = await cmsApi.deleteLeadsContact(id);
  ...
  const onRestore = async (id: string, name: string) => {
    ...
    setRestoringId(id);
    try {
      const res = await cmsApi.restoreLeadsContact(id);
  ...
  return (
    ...
    >
      <Card>
        <CardHeader>
          <CardTitle>
            {tab === "deleted" ? "Recently deleted" : "All contact leads"}
          </CardTitle>
          <CardDescription>
            {loading
              ? "Loading…"
                ...
                {rows.length === 0 ? (
                  ...
                  rows.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell>
                        <Link
                          href={`/admin/leads-contact/${row.id}`}
