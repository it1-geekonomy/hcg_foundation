...
export default function LegalPagesListPage({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const message = cmsErrorMessage(err, `Failed to load ${section.label}`);
      setError(message);
  ...
  return (
    <CmsListShell
      ...
      createHref={`${section.basePath}/new`}
      createLabel={`Add ${section.singular}`}
      tab={tab}
      onTabChange={switchTab}
      activeTabLabel={section.activeListLabel}
      error={error}
    >
      <Card>
        <CardHeader>
