...
export class TermsAndConditionsController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'List recently deleted terms and conditions (CMS trash)',
    description:
      'Soft-deleted terms only, newest first. Restore with POST /terms-and-conditions/:id/restore.',
  })
  ...
  async findDeleted(@Query() query: PaginationQueryDto) {
    ...
    return {
      ...
      message:
        result.meta.total === 0
          ? 'No deleted terms and conditions found'
          : 'Deleted terms and conditions fetched successfully',
      ...result,
    };
  }
