...
export class PrivacyPolicyController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'List recently deleted privacy policies (CMS trash)',
    description:
      'Soft-deleted policies only, newest first. Restore with POST /privacy-policy/:id/restore.',
  })
  ...
  async findDeleted(@Query() query: PaginationQueryDto) {
    ...
    return {
      ...
      message:
        result.meta.total === 0
          ? 'No deleted privacy policies found'
          : 'Deleted privacy policies fetched successfully',
      ...result,
    };
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Restore soft-deleted privacy policy (CMS / super-admin)',
    description: 'Clears deletedAt so the policy shows again in CMS and on the website if published.',
  })
  @ApiOkResponse({ type: PrivacyPolicy })
  @ApiUnauthorizedResponse({
    description: 'Missing, invalid, or expired bearer token',
  ...
  async restore(@Param('id', ParseUUIDPipe) id: string) {
    ...
    return {
      statusCode: HttpStatus.OK,
      message: 'Privacy policy restored successfully',
      data,
    };
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Soft-delete privacy policy (CMS / super-admin)',
